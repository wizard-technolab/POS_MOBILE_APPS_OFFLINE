...
class _CartScreenState extends State<CartScreen> {
  ...
  // ── Customer Note Dialog (Odoo POS style with preset tags) ──────────────
  void _showCustomerNoteDialog(BuildContext context) {
    // Preset tags — same as product note tags
    const presetTags = ['Wait', 'To Serve', 'Emergency', 'No Dressing'];

    final controller = TextEditingController(
      text: _cart.customerNoteNotifier.value,
    );

    // Pre-select tags that already exist in saved note
    final selectedTags = <String>{};
    for (final tag in presetTags) {
      if (_cart.customerNoteNotifier.value.contains(tag)) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => StatefulBuilder(
        // StatefulBuilder so tag highlight toggles without full screen rebuild
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Title row with back arrow (Odoo POS style)
                Row(
                  children: [
                    GestureDetector(
                ...
                const SizedBox(height: 16),
                // Preset tag chips — tap to toggle on/off
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: presetTags.map((tag) {
                    ...
                    return GestureDetector(
                      onTap: () {
                        setModalState(() {
                          if (isSelected) {
                            // Remove tag from selected set and from text
                            ...
                            // Add tag to selected set and append to text
                            selectedTags.add(tag);
                            final current = controller.text.trim();
                            controller.text =
                                current.isEmpty ? tag : '$current $tag';
                          }
                          // Move cursor to end after text change
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          // Purple tint when selected, default bg otherwise
                          color: isSelected
                        ...
                        child: Text(
                          tag,
                          style: TextStyle(
                            color: isSelected ? _kPurple : _kTextPrimary,
                            fontSize: 13,
                            fontWeight: FontWeight.w500,
                          ),
                        ),
                ...
                Row(
                  children: [
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () {
                          // Save customer note — customerNoteNotifier updates
                          // reactively so cart shows note instantly, no refresh needed
                          _cart.setCustomerNote(controller.text.trim());
                          Navigator.pop(ctx);
                        },
                        style: ElevatedButton.styleFrom(
                          backgroundColor: _kPurple,
